...
function [matched_color] = match_color(color)
	...
	YELLOW = [255, 213, 0];
	WHITE = [255, 255, 255];
	possible_colors = [RED; GREEN; BLUE; ORANGE; YELLOW; WHITE; color]

	tmp = zeros(1,7,3);
    for i=1:7
        tmp(1,i,:) = possible_colors(i,:);
    end
	possible_colors_lab = RGB2Lab(tmp);


	function d = ediff(color, orig)				
		[L a b] = rozdel(color);
		[L2 a2 b2] = rozdel(orig);
		d = sqrt( (L-L2)^2 + (a-a2)^2 + (b-b2)^2);
	end

	score = 1000000;		
	for i=1:6
		scr = ediff(possible_colors_lab(1,7,:), possible_colors_lab(1,i,:));
		if(score>scr)
			score = scr;
			matched_color = possible_colors(i,:);			
		end
	end
